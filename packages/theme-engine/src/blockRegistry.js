...
exports.resolveSectionBlocks = resolveSectionBlocks;
exports.resolveBlockSchema = resolveBlockSchema;
exports.buildBlockDefaultSettings = buildBlockDefaultSettings;
...
function resolveSectionBlocks(schema) {
    if (!schema) {
        return [];
    }
    const resolved = new Map();
    for (const blockType of schema.allowedBlockTypes || []) {
        const blockSchema = exports.BlockRegistry[blockType];
        if (blockSchema) {
...
function resolveBlockSchema(schema, blockType) {
    return resolveSectionBlocks(schema).find((block) => block.type === blockType);
}
function buildBlockDefaultSettings(blockSchema) {
    if (!blockSchema) {
        return {};
    }
