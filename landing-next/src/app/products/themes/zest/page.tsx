...
export default function ZestV2ThemePage() {
  const { t, lang } = useLanguage();

  // Statically assign Zest V2 data as this is a dedicated page
  const themeData = t.themeDetails?.zest2;
  ...
  return (
    ...
    >
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-6 pt-16 pb-24 text-start">
        ...
        {/* Full Image Gallery - Natural Scaling Performance & Targeted Cropping */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-24">
          {themeData.images.slice(1).map((img: string, idx: number) => {
            const isTargetImage = img.includes("screen_nr6gyz.png");
            return (
              <div key={idx} className="w-full">
                <MatgarcoImage
                  src={img}
                  alt={`Theme Showcase ${idx + 1}`}
                  aspectRatio={isTargetImage ? "square" : "auto"}
                  objectFit="cover"
                  imageClassName={isTargetImage ? "object-bottom" : ""}
