...
import MosaicHeader from './Header';
import MosaicFooter from './Footer';
...
export default function MosaicHomePage({ theme, merchant, products, isPreview }: Props) {
  const enabledSections = getEnabledSections(theme);
  return (
    <>
      {isPreview && (
        <div className="fixed top-0 left-0 right-0 z-[9999] bg-violet-600 text-white text-xs text-center py-2 font-medium">
          ⬜ وضع المعاينة — Mosaic Draft
        </div>
      )}
      <div className={`min-h-screen flex flex-col ${isPreview ? 'pt-8' : ''}`} style={{ backgroundColor: theme.colors.background }}>
        <MosaicHeader theme={theme} merchant={merchant} />
        ...
        <MosaicFooter theme={theme} merchant={merchant} />
      </div>
    </>
  );
}
