...
import EpureHeader from './Header';
import EpureFooter from './Footer';
...
export default function EpureHomePage({ theme, merchant, products, isPreview }: Props) {
  const enabledSections = getEnabledSections(theme);
  return (
    <>
      {isPreview && (
        <div className="fixed top-0 left-0 right-0 z-[9999] bg-amber-700 text-white text-xs text-center py-2 font-medium">
          🪡 وضع المعاينة — Épure Draft
        </div>
      )}
      <div className={`min-h-screen flex flex-col ${isPreview ? 'pt-8' : ''}`} style={{ backgroundColor: theme.colors.background }}>
        <EpureHeader theme={theme} merchant={merchant} />
        ...
        <EpureFooter theme={theme} merchant={merchant} />
      </div>
    </>
  );
}
