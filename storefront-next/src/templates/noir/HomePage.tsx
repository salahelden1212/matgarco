...
import NoirHeader from './Header';
import NoirFooter from './Footer';
...
export default function NoirHomePage({ theme, merchant, products, isPreview }: Props) {
  const enabledSections = getEnabledSections(theme);
  return (
    <>
      {isPreview && (
        <div className="fixed top-0 left-0 right-0 z-[9999] text-xs text-center py-2 font-medium" style={{ backgroundColor: '#B8973B', color: '#000' }}>
          ◆ وضع المعاينة — Noir Draft
        </div>
      )}
      <div className={`min-h-screen flex flex-col ${isPreview ? 'pt-8' : ''}`} style={{ backgroundColor: '#0A0A0A' }}>
        <NoirHeader theme={theme} merchant={merchant} />
        ...
        <NoirFooter theme={theme} merchant={merchant} />
      </div>
    </>
  );
}
